...
tryCatch({
  # Log script start
  message("Starting D01_01: Cleansing raw_data.df_amazon_sales to cleansed_data.df_amazon_sales")
  ...
  df_amazon_sales___cleansed <- raw_amazon_sales %>%
    ...
    mutate(
     ...
     # zipcode = stringr::str_trim(zipcode)
    ) %>%
  ...
  tryCatch({
    ...
    if (table_exists) {
      ...
      if (cleansed_row_count > 0) {
        ...
        required_columns <- c("amazon_order_id", "purchase_date", "time")
