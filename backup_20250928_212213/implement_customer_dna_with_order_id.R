# WISER Project: Customer DNA Implementation with Order ID
#
# This script implements the Customer DNA analysis for the WISER project
# using order_id as customer identifier since no email data is available.
# This will treat each order as a separate customer for demonstration purposes.
...
# Show available columns in raw data
message("Available columns in raw_data.amazon_sales_dta:")
raw_cols <- dbListFields(raw_data, "amazon_sales_dta")
print(raw_cols)

# Step 3: Prepare Customer Time Series Data
message("Preparing customer time series data...")

# Extract and transform raw data directly
message("Extracting and transforming data...")
sales_data <- tbl(raw_data, "amazon_sales_dta") %>%
  select(
    amazon_order_id,
    purchase_date,
    sku,
  ...
  rename(
    customer_id = amazon_order_id,  # Use order ID as customer ID for demonstration
    time = purchase_date,
...
if (nrow(sales_data) == 0) {
  stop("No sales data found. Please check your data source.")
}
...
# Save to cleansed database
dbWriteTable(cleansed_data, "amazon_sales_dta", sales_data, overwrite = TRUE)
message("Data saved to cleansed_data.amazon_sales_dta")
