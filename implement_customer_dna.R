...
# using the simplified cleansing approach that doesn't require product_property_dictionary.
...
if (!dbExistsTable(cleansed_data, "amazon_sales_dta")) {
  ...
  if (!dbExistsTable(raw_data, "amazon_sales_dta")) {
    stop("Raw amazon_sales_dta table not found. Please import data first.")
  }
...
# Handle possible date range issues more robustly
tryCatch({
  date_range <- c(min(sales_data$time, na.rm = TRUE), max(sales_data$time, na.rm = TRUE))
