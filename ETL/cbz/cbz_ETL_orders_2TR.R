...
tryCatch({
  ...
  orders_staged <- dbGetQuery(staged_data, sprintf("SELECT * FROM %s", staged_table))
  ...
  required_fields <- c(
    ...
    "platform_code", "transformation_timestamp"
  )
...
if (script_success) {
  tryCatch({
    ...
    required_cols <- c("order_id", "platform_code", "transformation_timestamp")
    ...
    if ("order_status" %in% columns) {
      ...
      status_check <- dbGetQuery(transformed_data, sprintf("
